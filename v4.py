...
def removeInFolders():
	...
	for i in movies.keys():
		...
		#['downloads/Despicable Me 2/Despicable.Me.2.2013.BRRip.Isl.Tal.avi']
		#['downloads/12 Years a Slave (2013) 1080p [ODI].mp4']
	...
	'''for i in movies.keys():
		for j in movies.values():
			...
			print(j)
			dst =  os.getcwd() + '/downloads/Movies/'+ i + '/'
			#isfolder =''.join(path[1:])

			print('shutil : ', src, dst)
			print('title: ' , i)
			#try:
			#	shutil.move(src, dst)
			#except:
			#	print('Gat ekki Fært: ' , src)

				#shutil.move(pathsString,)'''
